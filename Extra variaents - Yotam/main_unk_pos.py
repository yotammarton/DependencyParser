...
class DependencyDataset(Dataset):
    ...
    def init_pos_vocab(self):
        ...
        """
        idx_pos_mappings = [ROOT_POS, UNKNOWN_POS]
...
def word_dropout(model, data, alpha=0.25):
    ...
    for _, sample_tensor in data:
        ...
        unk_pos_tensor = torch.empty(n, dtype=torch.int64). \
            fill_(model.dataset.pos_idx_mappings['<unk_pos>'])
        ...
        # for tags
        sample_tensor[1][0] = torch.where(bernoulli_toss == 0,  # condition
                                          tags_tensor,  # if condition true take element from this tensor
                                          unk_pos_tensor)  # else take from this tensor
