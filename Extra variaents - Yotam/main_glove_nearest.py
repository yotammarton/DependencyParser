...
class DependencyDataset(Dataset):
    ...
    def convert_sentences_to_dataset(self):
        ...
        # source: https://www.cs.toronto.edu/~lczhang/360/lec/w06/w2v.html
        ...
        for sample_idx, sample in enumerate(self.datareader.D):
            ...
            for word in words:
                ...
                # word not in our vocabulary but in glove's vocabulary
                elif all(glove[word]):  # True = have at least one element which is not zero
                    # norm 2 dist
                    dists = torch.norm(glove.vectors - glove[word], dim=1)  # compute distances to all words
                    lst = sorted(enumerate(dists.numpy()), key=lambda x: x[1])  # sort by distance

                    # cosine similarity
                    # dists = torch.cosine_similarity(glove.vectors, glove[word].view(1, 100), dim=1)  # cosine
                    # lst = sorted(enumerate(dists.numpy()), key=lambda x: x[1], reverse=True)  # sort by distance

                    found_similar_word = False
                    for idx, difference in lst[1:n + 1]:  # take the top n
                        # a similar word is in our vocabulary
                        if glove.itos[idx] in self.word_idx_mappings:
                            # print(f'word = {word}, most_similar in our vocab = {glove.itos[idx]}')
                            words_idx_list.append(self.word_idx_mappings[glove.itos[idx]])
                            found_similar_word = True
                            break

                    # none of the top n words in our vocabulary >> assign UNKNOWN_TOKEN
                    if not found_similar_word:
                        words_idx_list.append(self.word_idx_mappings[UNKNOWN_TOKEN])

                # word not in our vocabulary ano not in glove's vocabulary
                else:
                    words_idx_list.append(self.word_idx_mappings[UNKNOWN_TOKEN])
...
def train_kiperwasser_parser(model, train_dataloader, test_dataloader, epochs, learning_rate, weight_decay, alpha):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, input_data in data:
            ...
            loss = loss / acumulate_grad_steps  # TODO??
            loss.backward()
